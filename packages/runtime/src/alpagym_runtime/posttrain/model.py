...
class AlpamayoTrainModel(nn.Module):
    ...
    def __init__(self, model_name_or_path: str, dtype: torch.dtype = torch.bfloat16) -> None:
        ...
        # {param name: rows before FSDP padding}. posttrain's weight export reads this off the
        # model by name (`comm/weight_transfer.py::_unpadded_rows`) and trims the pad after the
        # all-gather, so the rollout -- which never sharded -- is not handed pad rows as weights.
